...
SampleCommunication::updateCurrentSender( const PlayerAgent * agent )
{
    ...
    else
    {
        ...
        // exclude goalie
        //
        const int goalie_unum = ( wm.teammateGoalieUnum() != Unum_Unknown
                                  ? wm.teammateGoalieUnum()
...
SampleCommunication::sayBallAndPlayers( PlayerAgent * agent )
{
    ...
    // opponent goalie
    //
    if ( 1 <= wm.opponentGoalieUnum()
         && wm.opponentGoalieUnum() <= 11 )
    {
        int num = wm.opponentGoalieUnum() + 11;
    ...
    //
    {
        ...
        {
            {
                const AbstractPlayerObject * t = wm.teammate( unum );
                if ( ! t
                     || t->unumCount() >= 2 )
                {
            ...
            {
                const AbstractPlayerObject * o = wm.opponent( unum );
                if ( ! o
                     || o->unumCount() >= 2 )
                {
